...
class State:
	...
	def calculate_utility(self):
		...
		free_tiles_utility = self.free_tiles_utility() * 0.24389353395785351
		highest_tile_utility = self.highest_tile_utility() * 0.2
		largest_tile_corner_util = self.largest_tile_corner_util() * 0.2
		cluster_score = self.cluster_score() * 0.17666661624243543
		twos_fours = self.number_of_2s4s() * 0.21167771963406837
		number_of_same = self.number_of_same() * 0.2
		brute_method = self.brute_method() * 0.3
		brute_line2 =0# self.brute_line2() * 0
		upper_vs_lower = self.sum_greater_upper() * 0.18707119830211316
